...
var DropDownMenu = React.createClass({
		...
		this.props.eventIds.map(function (id, i) {
			if (selected[id]) {
				$.ajax({
					...
					url: 'https://api.tnyu.org/' + _this.state.API_VERSION + '/events/' + id + '/rsvp',
					async: false,
					dataType: 'jsonp'
				});
			}
	...
	render: function render() {
		...
		var itemNodes = this.props.eventTitles.map(function (title, i) {
			var dateObj = _this2._getTime(i);
			var isFull = _this2._isEventFull(i);
			var isRsvpd = _this2.props.rsvpdEvents[i];
			var checkbox = isFull || isRsvpd ? isRsvpd ? React.createElement(
				...
				null,
				'Event FULL'
			) : React.createElement('input', { type: 'checkbox', key: i, onChange: _this2._toggleCheckbox.bind(_this2, i) });
...
var AppHandler = React.createClass({
	...
	componentWillMount: function componentWillMount() {
		...
		$.getJSON('https://api.tnyu.org/' + this.state.API_VERSION + '/people/me').done(function (user) {
			...
			// get events
			$.getJSON('https://api.tnyu.org/' + _this3.state.API_VERSION + '/events/upcoming-publicly?page%5Blimit%5D=10&sort=startDateTime').done(function (json) {
				var eventIds = [],
				    eventTitles = [],
				    eventStartDates = [],
				    venueIds = [],
				...
				json.data.map(function (event) {
					var alreadyRsvpd = event.relationships.rsvps.data.some(function (person) {
						return person.id.toString() === _this3.state.userId.toString();
					});
				...
				var venueNames = _this3.state.venueNames,
				    venueAddresses = _this3.state.venueAddresses,
				    venueCaps = _this3.state.venueCaps;
				_this3.state.venueIds.map(function (venueId) {
					$.getJSON('https://api.tnyu.org/' + _this3.state.API_VERSION + '/venues/' + venueId.toString()).done(function (json) {
